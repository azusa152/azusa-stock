...
import { useBacktestDetail, useBacktestSummary } from "@/api/hooks/useBacktest"
          ...
          return (
            <Card key={signal.signal} className="border-border">
                ...
                <CardTitle className="text-sm flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      ...
                      signal.confidence === "high" && "bg-emerald-500/20 text-emerald-400",
                      signal.confidence === "medium" && "bg-amber-500/20 text-amber-300",
              ...
              <CardContent className="space-y-1 text-xs">
                <p>{t("backtest.card.direction")}: {signal.direction}</p>
                <p>{t("backtest.card.hit_rate_30d")}: {((metric30?.hit_rate ?? 0) * 100).toFixed(1)}%</p>
                <p>{t("backtest.card.avg_return_30d")}: {(metric30?.avg_return_pct ?? 0).toFixed(2)}%</p>
                <p>{t("backtest.card.samples_30d")}: {metric30?.sample_count ?? 0}</p>
              </CardContent>
            </Card>
          )
        })}
        ...
        <CardContent className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={hitRateData} layout="vertical">
              ...
              <YAxis dataKey="signal" type="category" tick={{ fill: theme.tickColor, fontSize: 11 }} width={120} />
              <Tooltip
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/30">
                <tr>
                  ...
                  <th className="text-left px-3 py-2">{t("backtest.table.market_status")}</th>
                  {WINDOWS.map((days) => (
                    <th key={days} className="text-right px-3 py-2">{days}d</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(detail?.occurrences ?? []).map((occ, idx) => (
                  <tr key={`${occ.ticker}-${occ.signal_date}-${idx}`} className="border-t border-border">
